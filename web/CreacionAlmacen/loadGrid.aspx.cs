...
namespace JQuery
{
    public partial class loadGrid : System.Web.UI.Page
    {
        ...
        private void fillTable(string rack, string zona)
        {
            ...
            int n = 0, v = 0, pos = 0, idPosicion=0;
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    for (int i = 0; i < 1; i++)
                    {
                        ...
                        ",P.imo, replace(P.peso,',','.'), P.altura, P.IdTipoPos , P.Clave " +
                        "FROM RACKS R " +
                        ...
                        "WHERE N.Clave = " + n + " AND R.IDZona = " + ddlZona.SelectedValue.ToString() + " AND R.IDRack = '" + ddlRack.SelectedItem.Value + "' AND P.IdTipoPos = 1 ORDER BY v.idVentana";
                        com2 = new SqlCommand(scom, con2);
                        SqlDataReader dr2 = com2.ExecuteReader();
                        while (dr2.Read())
                        {
                            vent = dr2[4].ToString(); //String.Format("{0:00}", dr2[4]);
                            idPosicion = int.Parse(dr2[6].ToString());
                            pos = int.Parse(dr2[11].ToString());
                            id = ddlZona.SelectedItem.ToString()+dr2[1].ToString() + n + vent +","+ idPosicion+","+pos;
        ...
        protected void Button3_Click(object sender, EventArgs e)
        {
            ...
            string  altura = Request.QueryString["altura"];
            string  clave   = Request.QueryString["clave"];
            string  nivel   = clave.Substring(3, 1);
            string[] cadenaPartida = clave.Split(',');
            string ventana = cadenaPartida[1];
        ...
        private void setPosiciones(string nivel, string ventana, int pos, string imo, string peso, string altura, string tipoPos) {
            ...
            {                
                ...
                "inner join niveles n on n.IDNivel = v.IDNivel " +
                "where v.IDVentana = '" + ventana + "' AND n.clave = " + nivel + " ORDER BY clave desc";
                con = new SqlConnection(ds);
                com = new SqlCommand(scom, con);
                con.Open();
                SqlDataReader dr = com.ExecuteReader();

...
namespace JQuery
{
    public partial class Zona : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            //}
            llenaCBConexion(ddlSucursal, "Nombre", "Sucursal", "SELECT Nombre, Sucursal from Sucursal where Nombre LIKE '%fab%'","Intelisis");
        ...
        protected void btnZonas_Click(object sender, EventArgs e)
        {
            ...
            int cantZonas = 0;
            int conteo = 0;
            string idAlmacen = ddlAlmacen.SelectedValue.ToString();
            try
            {
                ...
                else
                {
                    ...
                    for (int x = 0; x < cantZonas; x++)
                    {
                        ...
                        {
                            conteo += con.insert("INSERT INTO Zonas VALUES('" + ABC[x] + "',' ','" + idAlmacen + "')");
                        }
                    }
                    if (conteo == 0)
                    {
        ...
        public bool generar()
        {
            ...
            try
            {
                if (limpiar())
                {
                    ...
                    cuenta += con.Update_Int("CREATE TABLE Zonas( IdZona int identity(1,1) primary key, ClaveZona char not null, Descripcion varchar(30)not null, idSucursal varchar(10) not null)");
        ...
        public void llenaCB(DropDownList Objeto, string nomCve, string idCve, string consulta)
        {
            DataSet llena = ws.getDataset(consulta);
            DataTable zonas = llena.Tables[0];
            Objeto.DataSource = null;
            Objeto.DataSource = zonas;
            Objeto.DataTextField = nomCve;
            Objeto.DataValueField = idCve;
            Objeto.DataBind();
        ...
        public void llenaCBConexion(DropDownList Objeto, string nomCve, string idCve, string consulta, string Conexion)
        {
        ...
        {
            ddlAlmacen.Visible = true;
            lblAlmacenes.Visible = true;
            //string valSucursal = ddlSucursal.SelectedValue.ToString();
            llenaCBConexion(ddlAlmacen, "Nombre", "Almacen", "select * FROM Alm where Sucursal = '" + ddlSucursal.SelectedValue.ToString() + "'", "Intelisis");
            lblddl1.Text = ddlSucursal.SelectedItem.ToString() + " " + ddlSucursal.SelectedValue.ToString()+" "+ddlSucursal.SelectedIndex.ToString();
        }

        protected void ddlAlmacen_SelectedIndexChanged(object sender, EventArgs e)
        {
            lblddl2.Text = ddlAlmacen.SelectedItem.ToString() + " " + ddlAlmacen.SelectedValue.ToString() + " " + ddlAlmacen.SelectedIndex.ToString();
        }
    }
}